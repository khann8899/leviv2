  ...
  bot.onText(/^\/portfolio$/, async (msg) => {
    ...
    if (manual.length > 0) {
      ...
      for (const p of manual) {
        const mult = p.currentPrice / p.entryPrice;
    ...
    try {
      ...
      for (const token of newTokens) {
        ...
        if (
          ...
          Object.keys(state.autoPositions).length < state.autobetSlots
        ) {
          console.log(`🤖 Autobet triggered for ${token.symbol}`);
